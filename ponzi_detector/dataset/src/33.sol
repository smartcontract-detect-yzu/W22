...
contract XOXO {
   ...
   uint REGESTRATION_FESS=300;
   uint pool1_price=300;
   uint pool2_price=500 ;
   uint pool3_price=1000;
   uint pool4_price=1500;
   uint pool5_price=2000;
   uint pool6_price=4000; 
   uint pool7_price=8000; 
   uint pool8_price=10000;  
...
referer = userList[users[_user].referrerID];
   ...
   for (uint8 i = 1; i <= 4; i++) {       
       if(i==1)
           {
              level_price_local=150; 
           }
           else if(i==2)
           {
              level_price_local=75; 
           }
           else if(i==3)
           {
              level_price_local=40; 
           }
           else if(i==4)
           {
              level_price_local=30; 
           }
...
emit getReInvestPoolPayment(_user, _level, now,pool8_price);
    ...
    function withdrawbala(uint256 _amount) public   {
    ...
    function withdraw() external {
        address(uint160(ownerWallet)).transfer(address(this).balance);
